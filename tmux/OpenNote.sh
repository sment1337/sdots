...
var2=$(egrep --recursive --with-filename --line-number --binary-files=without-match $var1 'DripBox/QownNotes' | fzf --preview-window 'right,70%' --bind "ctrl-p:preview(sh ~/sdots/tmux/previewNote.sh {} $var1),ctrl-/:change-preview-window(hidden),pgdn:preview-page-down,pgup:preview-page-up")
